...
export function SaveDataProvider({ children }: { children: ReactNode }) {
    ...
    setSaveData((prevSaveData) => {
      ...
      if (building.assigned !== undefined) {
        ...
        newPopulation = {
          ...
          availableWorkers: prevSaveData.population.availableWorkers - 1,
        };
  ...
  }, [JSON.stringify(saveData.buildings), JSON.stringify(saveData.resources)]);
  ...
  function checkVisibilityConditions() {
    setSaveData((prevSaveData) => {
      ...
      const updatedBuildings = checkBuildingConditions(buildings, resources);
      const updatedResources = checkResourceConditions(resources, buildings);
