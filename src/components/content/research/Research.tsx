...
function Research() {
    ...
    function isPrerequisiteMet(
        ...
        if (prerequisite in saveData.research) {
            return saveData.research[prerequisite as keyof IResearch]
                .isComplete;
        } else if (prerequisite in saveData.buildings) {
            return (
                saveData.buildings[prerequisite as keyof Buildings].owned > 0
            );
        } else if (prerequisite in saveData.resources) {
            return !saveData.resources[prerequisite as keyof Resources]
                .isHidden;
        }
        return false;
    }

    const isViewingPrerequisiteMet = (
    ...
    return (
        <Tabs w="100%" align="center">
            ...
            <TabPanels>
                <TabPanel>
                    <ResearchContainer>
                        {Object.keys(saveData.research).map((key) => {
                            const researchData = AllResearchData.get(
                                key as keyof IResearch
                            );
                            if (!researchData) return null;

                            if (
                                !isViewingPrerequisiteMet(
                                    researchData.viewingPrerequisites
                                )
                            ) {
                                return null;
                            }
